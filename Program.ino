...
void loop() 
{
  ...
  duration1 = pulseIn(echoPin1, HIGH);        // Receive Reflected Waves, done using HC-SR04 sensors
  distance1 = duration1 / 58.2;                       // Get Distance
  Serial.println(distance1);
  delay(5); 
  
  
  if (distance1 < 10 )               // Condition when no edge is detected           
  {
  ...
  else if (distance1>15)            // Condition when edge is detected  
  {
 ...
 if (distance2 < 10  )               // Condition when no edge is detected             
  {
    digitalWrite(MRb, HIGH);       // Move Forward
    digitalWrite(MRa, LOW);
    digitalWrite(MLb, HIGH);                                
  ...
  else if (distance2>15 )            // Condition when edge is detected  
  {
    digitalWrite(MRb, LOW);     //Stop                
    digitalWrite(MRa, LOW);
    digitalWrite(MLb, LOW);                                
